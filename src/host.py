...
def generate_metric(seq, metrics, mapping, birth=False, timestamp=int(time.time())):
    ...
    for name, value in metrics.items():
        metric(payload1, None if mapping == None or name not in mapping else mapping[name], value, name, birth)
...
class SparkplugHost:
	...
	def __init__(self, config):
		...
		for group in config["groups"]:
			...
			self.client.message_callback_add("spBv1.0/" + group + "/NBIRTH/#", self.handle_action)
			self.client.subscribe("spBv1.0/" + group + "/DBIRTH/#")
			self.client.message_callback_add("spBv1.0/" + group + "/DBIRTH/#", self.handle_action)
			self.client.subscribe("spBv1.0/" + group + "/NDATA/#")
			self.client.message_callback_add("spBv1.0/" + group + "/NDATA/#", self.handle_action)
			self.client.subscribe("spBv1.0/" + group + "/DDATA/#")
			self.client.message_callback_add("spBv1.0/" + group + "/DDATA/#", self.handle_action)
			self.client.subscribe("spBv1.0/" + group + "/NDEATH/#")
			self.client.message_callback_add("spBv1.0/" + group + "/NDEATH/#", self.handle_action)
			self.client.subscribe("spBv1.0/" + group + "/DDEATH/#")
			self.client.message_callback_add("spBv1.0/" + group + "/DDEATH/#", self.handle_action)
	...
	def handle_action(self, client, userdata, msg):
		group_name, node_name, device_name, action, payload = self.extract_msg(msg)
		if action == "NBIRTH":
	...
	def handle_nbirth(self, group_name, node_name, payload):
		print("NBIRTH: " + node_name)
		self.edgeNodeSeq[group_name + node_name] = payload.seq
		self.edgeNodeAlive[group_name + node_name] = True
		storage.insert_group(group_name)
		storage.insert_edge_node(group_name, node_name, "ONLINE", payload.timestamp, 0)
	
	def handle_dbirth(self, group_name, node_name, device_name, payload):
		print("DBIRTH: " + node_name + "/" + device_name)
		storage.insert_device(group_name, node_name, device_name, "ONLINE", payload.timestamp, 0)
		for metric in payload.metrics:
			metric_type_str, value = get_metric_type_string(metric)
			storage.insert_metric(group_name, node_name, device_name, metric.name, metric_type_str)
			metric_id = storage.get_metric_id(group_name, node_name, device_name, metric.name)
			storage.insert_metric_value(metric_id, metric_type_str, value, payload.timestamp)


	def handle_ndata(self, group_name, node_name, payload):
		print("NDATA: " + node_name)
	...
	def handle_ndeath(self, group_name, node_name, payload):
		...
		storage.update_edge_node_status(group_name, node_name, "OFFLINE", payload.timestamp)

	def handle_ddeath(self, group_name, node_name, device_name, payload):
		print("DDEATH: " + node_name + "/" + device_name)
		storage.update_device_status(group_name, node_name, device_name, "OFFLINE", payload.timestamp)
